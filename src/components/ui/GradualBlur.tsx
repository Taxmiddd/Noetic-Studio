...
const useResponsiveDimension = (responsive: boolean, config: any, key: string) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v = config[key];
      const suffixedKey = key[0].toUpperCase() + key.slice(1);
      if (w <= 480 && config[`mobile${suffixedKey}`])
        v = config[`mobile${suffixedKey}`];
      else if (w <= 768 && config[`tablet${suffixedKey}`])
        v = config[`tablet${suffixedKey}`];
      else if (w <= 1024 && config[`desktop${suffixedKey}`])
        v = config[`desktop${suffixedKey}`];
      setValue(v);
    };
